...
impl CPU {
    ...
    fn execute(&mut self, instruction: Opcode) -> Result<(u16, u32), CpuError> {
        match instruction {
            ...
            0x10 => Ok((self.pc.wrapping_add(1), 4)), // STOP
            0x76 => Ok((self.pc.wrapping_add(1), 4)), // HALT

            /* 8-bit load */
            0x40 => Ok((self.pc.wrapping_add(1), 4)), // LD B, B
            0x41 => {
                // LD B, C
            ...
            0xF8 => {
                ...
                self.reg
                    .f
                    .set_h((self.sp & 0xfff) + (n as u16 & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
                Ok((self.pc.wrapping_add(2), 12))
            }
            0x08 => {
            ...
            0x09 => {
                ...
                self.reg
                    .f
                    .set_h((self.reg.hl() & 0xfff) + (self.reg.bc() & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
                Ok((self.pc.wrapping_add(1), 8))
            }
            0x19 => {
                ...
                self.reg
                    .f
                    .set_h((self.reg.hl() & 0xfff) + (self.reg.de() & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
                Ok((self.pc.wrapping_add(1), 8))
            }
            0x29 => {
                ...
                self.reg
                    .f
                    .set_h((self.reg.hl() & 0xfff) + (self.reg.hl() & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
                Ok((self.pc.wrapping_add(1), 8))
            }
            0x39 => {
                // ADD HL, SP
                let (result, overflow) = self.reg.hl().overflowing_add(self.sp);
                self.reg.set_hl(result);
                self.reg.f.set_s(false);
                self.reg
                    .f
                    .set_h((self.reg.hl() & 0xfff) + (self.sp & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
                Ok((self.pc.wrapping_add(1), 8))
            }
            0xE8 => {
                ...
                self.reg
                    .f
                    .set_h((self.sp & 0xfff) + (s as u16 & 0xfff) > 0xfff);
                self.reg.f.set_c(overflow);
